...
if strcmp(options.day, 'day1')
    load ([options.root, sprintf('randomWalks/133/combined_12/%u.mat',mod(data.subjNo,100))])
    seqOrder = wO(randperm(length(wO)));
    exposure
    bl = 1;    
else
    seqOrder = randi(40,10,1); while numel(seqOrder)<options.testblocks, seqOrder = randi(40,10,1);end
    ...
    cgtext('There will be ten blocks today.' ,0,-90)
...
while  bl <= limit
    ...
    if strcmp(options.day, 'day2') && length(data.train{bl}.seq) ~= options.testBlocklength
        cgtext('Achtung! Falsche Trialzahl!',0,150)
        waitkeydown(inf)
    end
...
if strcmp(options.day, 'day2')
    totCr = sum(data.scan{bl}.correct);
%     cgfont('Arial', 50)
%     cgtext(sprintf('You will be paid £38.00 + £%.2f',totCr/10),0,100)
    cgfont('Arial', 30)
    cgtext('Thank you very much for participating',0,-50)
end
